...
for brand, data in car_data.items():
    ...
    if year >= year_min and engine >= engine_min and price <= price_max:
        filtered_cars.append((brand, year, engine, price))
filtered_cars.sort(key=lambda x: x[3])

for car in filtered_cars[:5]:
    brand, year, engine, price = car
    print(f"{brand}: year = {year}, engine = {engine}, price = {price}")
